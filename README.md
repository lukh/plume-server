...
* A Linux server (preferably Debian or some variant thereof)
...
>
> mkcert -install
> cd snv-server/ssl-proxy
> mkcert HOSTNAME IP
It creates a cert.pem and key.pem in ssl-proxy

Update ssl_certificate/ssl_certificate_key in ssl-proxy/default.conf
and server name entries
